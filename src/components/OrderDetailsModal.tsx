...
const OrderDetailsModal = ({ order, isOpen, onClose }: OrderDetailsModalProps) => {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-2xl bg-white dark:bg-slate-900 border-gray-200 dark:border-slate-800">
                <DialogHeader>
                    <DialogTitle className="text-2xl font-bold flex items-center gap-2">
                        <Package className="text-blue-600" />
                        Order Details <span className="text-gray-400 font-normal">#{order.id}</span>
                    </DialogTitle>
                    <DialogDescription>
                    ...
                    <div className="space-y-6">
                                ...
                                {order.items.map((item) => (
                                    <div key={item.id} className="flex gap-4">
                                        <img
                                            src={item.image}
                                            alt={item.name}
                                            className="w-16 h-16 rounded-md object-cover border border-gray-100 dark:border-slate-800"
                                        />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div className="text-sm text-gray-600 dark:text-gray-300 pl-6 border-l-2 border-gray-100 dark:border-slate-800">
                                    <p>John Doe</p>
                                    <p>123 Tech Avenue</p>
